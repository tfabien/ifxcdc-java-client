...
public class CDCMessageFactory {
	...
	public static Vector<CDCRecord> createCDCRecords(byte[] data, int availableBytes, CDCConnection cdcConnection) {
		Vector<CDCRecord> records = new Vector<CDCRecord>();
		int index = 0;
		while (index < availableBytes) {
			byte[] b = Arrays.copyOfRange(data, index, availableBytes);
			CDCRecord record = createCDCRecord(b, cdcConnection);
			records.add(record);
	...
	public static CDCRecord createCDCRecord(byte[] data, CDCConnection cdcConnection) {
	...
	public static Map<String, Object> parseCDCRecordPayload(CDCOperationRecord cdcOperationRecord, CDCMetadataRecord cdcMetadataRecord) {
		...
		Map<String, Object> map = new HashMap<String, Object>(totalCols);
		...
		for (int i = 0; i < cdcMetadataRecord.getFixedLengthCols(); i++) {
			...
			Object colValue;
			try { colValue = parseFixedLengthCol(payload, colType, colLength, colPrec ); } catch (Exception e) { colValue = e.getMessage(); }
		...
		for (int i = cdcMetadataRecord.getFixedLengthCols(); i < totalCols; i++) {
			...
			String colType = cdcMetadataRecord.getColTypes().get(colName);
			Object colValue;
			try { colValue = parseVariableLengthCol(payload, colType); } catch (Exception e) { colValue = e.getMessage(); }
			log.debug("{} ({}): {}", colName, colType, colValue);
			map.put(colName, colValue);
		}
		return map;
